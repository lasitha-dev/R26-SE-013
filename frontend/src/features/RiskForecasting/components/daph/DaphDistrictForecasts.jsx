...
export function DaphDistrictForecasts({ viewerContext }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [recordsRes, districtsRes] = await Promise.allSettled([
        listForecastRecords({ limit: 200, target_year: selectedYear, target_month: selectedMonth }, { signal: controller.signal }),
      ...
      if (recordsRes.status === 'fulfilled') {
        const rawRecords = recordsRes.value?.records || [];
        fetchedRecords = rawRecords.filter(
          r => (r.target_year ?? r.targetYear) === Number(selectedYear) && (r.target_month ?? r.targetMonth) === Number(selectedMonth)
        );
      }

      // Fall back to live forecast evaluation using the trained model pipeline for any month/year if no saved DB records exist
      if (fetchedRecords.length === 0) {
        try {
          const liveRes = await fetchAuthorizedDiseaseForecasts({ year: selectedYear, targetMonth: selectedMonth, signal: controller.signal });
          if (controller.signal.aborted) return;
          const liveRecords = [];
          ...
          if (liveRecords.length > 0) {
            fetchedRecords = liveRecords;
          }
        } catch (_) {
          // If live request fails, fallback stays empty
        }
      }
  ...
  }, [authorizedDistricts, isNational, selectedYear, selectedMonth]);
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-1">
            <h1 className="text-2xl font-bold text-primary tracking-tight">
              {MONTH_NAMES[selectedMonth - 1]} {selectedYear} District Disease Risk Outlook
            </h1>
...

      {
        !loading && !error && displayRecords.length === 0 && (
          <div className="p-8 text-center rounded-2xl bg-surface-container border border-outline-variant/30 space-y-3">
            <p className="text-on-surface font-medium">No district forecast records are available for selected period.</p>
          </div>
        )
      }
                  ...
                  <div className="flex-1 grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div className="space-y-1">
                      ...
                      <span className="text-on-surface font-medium">
                        {MONTH_NAMES[(r.target_month || selectedMonth) - 1]} {r.target_year || selectedYear}
                      </span>
                    </div>
                    <div className="space-y-1">
